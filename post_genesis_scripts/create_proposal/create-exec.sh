...
zetacored tx group submit-proposal post_genesis_scripts/create_proposal/proposal_whitelist_erc20.json --from $signer --chain-id=$CHAINID --keyring-backend=$KEYRING -y --broadcast-mode=block --node=$node --gas=auto --gas-adjustment=1.5 --gas-prices=0.01azeta
#sleep 5
